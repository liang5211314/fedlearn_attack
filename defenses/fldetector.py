...
class FLDetector(FedAvg):
    ...
    def aggr(self, weight_accumulator, global_model: nn.Module):
        ...
        for user in self.params.round_participants:
            if user.user_id in self.exclude_list:
                logger.info(f"FL-Detector: Skip client {user.user_id}")
                continue
        ...
        if distance is not None and \
         self.current_epoch - self.start_epoch > window_size:
            self.malicious_score = np.row_stack((self.malicious_score, distance[:self.malicious_score.shape[1]]))
            # self.malicious_score = np.row_stack((self.malicious_score, distance.reshape(-1, 1)))
            # self.malicious_score = np.row_stack((self.malicious_score, distance[:, np.newaxis]))
             ...
             n=total_participants-len(self.exclude_list)) >= 2:
                ...
                for index,user in enumerate(self.params.round_participants):
                    if label_pred[index]==0:
                        self.exclude_list.append(user.user_id)
...
#             v = 0
#             if v_k > 0:
#                 gap_value = v - math.log(v_k)  # Avoid log(0)
#                 gap.append(gap_value)
#             else:
#                 gap.append(float('-inf'))  # Handle case if v_k <= 0
#             for v_kb_i in v_kb:
#                 v += math.log(v_kb_i)
#             v /= num_sampling
...
def gap_statistics(data, num_sampling, K_max, n):
    ...
    # Linear transformation (normalizing data)
    data_c = np.ndarray(shape=data.shape)
    for i in range(data.shape[1]):
    ...
    for k in range(1, K_max + 1):
        # Fit KMeans for the given k
        k_means = KMeans(n_clusters=k, init='k-means++').fit(data_c)
        predicts = k_means.labels_.tolist()
        centers = k_means.cluster_centers_
        v_k = 0

        # Calculate the intra-cluster distances (v_k)
        for i in range(k):
            for idx, predict in enumerate(predicts):
                if predict == i:
                    v_k += np.linalg.norm(centers[i] - data_c[idx])

        # Generate synthetic data to calculate the gap
        v_kb = []
        for _ in range(num_sampling):
            data_fake = np.random.uniform(0, 1, size=(n, data.shape[1]))
            k_means_b = KMeans(n_clusters=k, init='k-means++').fit(data_fake)
            predicts_b = k_means_b.labels_.tolist()
            centers_b = k_means_b.cluster_centers_
            v_kb_i = 0
            for i in range(k):
                for idx, predict in enumerate(predicts_b):
                    if predict == i:
                        v_kb_i += np.linalg.norm(centers_b[i] - data_fake[idx])
            v_kb.append(v_kb_i)

        # Calculate the gap for this k
        if v_k > 0:
            gap_value = math.log(np.mean(v_kb) + 1e-10) - math.log(v_k + 1e-10)
            gap.append(gap_value)
        else:
            gap.append(float('-inf'))  # Handle cases where v_k is 0 or negative

        # Calculate the standard deviation (s)
        v = np.mean(np.log(np.array(v_kb) + 1e-10))  # Use log for gap consistency
        s_k = np.sqrt(np.mean((np.log(np.array(v_kb) + 1e-10) - v) ** 2)) * math.sqrt((1 + num_sampling) / num_sampling)
        s.append(s_k)
